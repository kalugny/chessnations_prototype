...
var P4WN_IMAGE_DIR = '/static/images/';
...
function update_msg(status){
	$("#message").text(status);
}

function update_awards(awards){
	for (var i = 0; i < awards.length; i++){
		$("#award-list").append('<li>' + awards[i].name + ': ' + awards[i].points + ' pts.</li>');
	}
}

_p4d_proto.refresh = function(force_update){
	if (typeof(force_update) === undefined){
		force_update = false;
	}
	var game = this;
	var game_id = this.game_id;
	$.getJSON('/game/'+game_id+'/status/', function(data) {
		var old_state = p4_state2fen(game.board_state)
		if ((old_state != data.fen) || force_update){
			game.board_state = p4_fen2state(data.fen);
			update_msg(data.status);
			$('.'+P4WN_LOG_CLASS).html('');
			for (var i = 0; i < data.moves.length; i++){
				game.log(data.moves[i], "p4wn-log-move");
			}
			game._refresh();
		}
	});
...
_p4d_proto.square_clicked = function(square){
    ...
    if (this.start == square){
		...
		this.move(this.start, square, P4WN_PROMOTION_INTS[this.pawn_becomes]);
    }
};
...
_p4d_proto.move = function(start, end, promotion){
	...
	$.getJSON(
			'move/', 
			{start: p4_stringify_point(start), end: p4_stringify_point(end), promotion: promotion, time_elapsed: 60}, 
			function(data){ 
				if (data.status === 'OK'){
					game._move(start, end, promotion);
					/*If the move works, drop the piece.*/
					game.stop_moving_piece(end);

				}
				else {
					game.stop_moving_piece(start);
				}
...
_p4d_proto._move = function(start, end, promotion){

...
class Board(object):
	...
	def move(self, move):
		...
		if piece.name == 'Pawn' and move.destination[0] == piece.promotion_row:
		...
		if self.turn == 'white':
			self.fullmove += 1
		move.set_pgn(piece, self.captured, detail, not self.not_in_check(), self.check_position() == 'checkmate')
